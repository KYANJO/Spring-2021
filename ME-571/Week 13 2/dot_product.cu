...
__global__ void dotProductOnGPU(int *A, int *B, int *C, const int N)
{
	...
	block_c += temp[j];
      atomicAdd(C,block_c); //add the resulting (block local) sum to global C
    }
...
__global__ void dotProductOnGPU_sharedAtomic(int *A, int *B, int *C, const int N)
{
  ...
  __shared__ int c_block;
    ...
    if(i<N) {
      temp = A[i] * B[i]; //each thread computes one entry of temp (no data race)
      atomicAdd(&c_block,temp); //add numbers within a block
    }
    ...
    if(threadIdx.x ==0) { //thread 0 from each block adds the block result to global sum C
      atomicAdd(C,c_block); 
    }

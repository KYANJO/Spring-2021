...
int main(int argc, char * argv[]){
  ...
  //for(int test=0; test<2; test++){ //timing trick - not needed for actual simulations
    ...
    **********/
	...
	u2[i] = alpha2*(u1[i-1] - 2*u1[i] + u1[i+1]) + 2*u1[i] - u0[i];
      ...
      //send right boundary, receive right ghost
      //MISSING CODE HERE
      
      //compute right end points
      //MISSING CODE HERE
      
      //make sure right domain boundary uses correct boundary condition
      //MISSING CODE HERE
	u2[npts_loc-1] = exact_solution(x[npts_loc-1],time,c);
      
      //send left boundary, receive left ghost
      //MISSING CODE HERE
      ...
      //MISSING CODE HERE
	u2[0] = exact_solution(x[0],time,c);

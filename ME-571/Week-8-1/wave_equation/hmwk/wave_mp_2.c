...
int main(int argc, char * argv[]){
  ...
  wtime = MPI_Wtime();
    ...
    **********/
	...
	MPI_Sendrecv(&ghost_right,1,MPI_DOUBLE,irank+1,101,&ghost_left,1,MPI_DOUBLE,nproc-1,101,MPI_COMM_WORLD,&status);
      }
      else if(irank==nproc-1 && nproc>1){
	MPI_Sendrecv(&ghost_right,1,MPI_DOUBLE,0,101,&ghost_left,1,MPI_DOUBLE,irank-1,101,MPI_COMM_WORLD,&status);
      }
	...
	MPI_Sendrecv(&ghost_left,1,MPI_DOUBLE,nproc-1,102,&ghost_right,1,MPI_DOUBLE,irank+1,102,MPI_COMM_WORLD,&status);
      }
      else if(irank==nproc-1 && nproc>1){
	MPI_Sendrecv(&ghost_left,1,MPI_DOUBLE,irank-1,102,&ghost_right,1,MPI_DOUBLE,0,102,MPI_COMM_WORLD,&status);
      }
...
void print_results(double *u, double *x, int npts, int npts_loc, int irank, int nproc, double time, double c)
  {
    ...
    if(irank==0){
      FILE *f;
      f = fopen("results_wave.csv","a"); //open file

...
int main(int argc, char * argv[]){
  ...
  double wtime, tcomm,tcomp, timer_comput_start, timer_comput;
...
//declare variables for tracking requests
  ...
  timer_comput_start = MPI_Wtime();
   ...
   timer_comput = MPI_Wtime() - timer_comput_start;
...
// COMMUNICATION COMPLETE         
 ...
 //complete computation at the rank edges (except the domain boundary conditions)
  ...
  MPI_Reduce(&wtime, &tcomm, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

  MPI_Reduce(&timer_comput, &tcomp, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
  
  if(irank == 0){
    printf("%d,%d,%f,%f\n",nproc,N_loc,tcomm,tcomp);
  }
...
void write_results(double *u, int N, int N_loc, int nproc, int irank, double h, double L){
  ...
  if(irank==0){
    ...
    //write to file
    FILE *f;
    f = fopen("results_mastery.dat","w"); //open file
    fprintf(f,"x, y, u\n");

...
int main(int argc, char * argv[]){
  ...
  int q = (int)sqrt(nproc); //N should be divisible by p            
  int p = q*q;
  int rank_row = irank/p;
  int rank_col = irank%p;
  ...
  double *send_buffer_rgtv = malloc(N_loc*sizeof(double));
  double *send_buffer_lftv = malloc(N_loc*sizeof(double));
  // double *send_buffer_botv = malloc(N_loc*sizeof(double));
  //  double *send_buffer_topv = malloc(N_loc*sizeof(double));

  double *recv_buffer_rgtv = malloc(N_loc*sizeof(double));
  double *recv_buffer_lftv = malloc(N_loc*sizeof(double));
  //double *recv_buffer_botv = malloc(N_loc*sizeof(double));
  //double *recv_buffer_topv = malloc(N_loc*sizeof(double));


  //initialize x and y
  for(i=1;i<N_loc+1;i++){
    x[i] = (i-(N_loc+1)/2.0)*h + rank_col*N_loc*h;
    y[i] = (i-(N_loc+1)/2.0)*h + rank_row*N_loc*h;//y coordinates are the same as x, but need not be
  }
  ...
  for(int time=1;time<=ntime;time++){
    ...
    */
    ...
    if(rank_col==p-1){
      ...
      MPI_Recv(recv_buffer_rgtv,N_loc,MPI_DOUBLE,rank_rgt,101,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    else if(rank_col==0){
      MPI_Send(send_buffer_lftu,N_loc,MPI_DOUBLE,rank_lft,101,MPI_COMM_WORLD);
      MPI_Send(send_buffer_lftv,N_loc,MPI_DOUBLE,rank_lft,101,MPI_COMM_WORLD);
    }
    else{
      MPI_Sendrecv(send_buffer_rgtu,N_loc,MPI_DOUBLE,rank_rgt,101,recv_buffer_lftu,N_loc,MPI_DOUBLE,rank_lft,101,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      MPI_Sendrecv(send_buffer_rgtv,N_loc,MPI_DOUBLE,rank_rgt,101,recv_buffer_lftv,N_loc,MPI_DOUBLE,rank_lft,101,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    ...
    if(rank_col==0){
      ...
      MPI_Recv(recv_buffer_lftv,N_loc,MPI_DOUBLE,rank_lft,102,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    else if(rank_col==p-1){
      MPI_Send(send_buffer_rgtu,N_loc,MPI_DOUBLE,rank_rgt,102,MPI_COMM_WORLD);
      MPI_Send(send_buffer_rgtv,N_loc,MPI_DOUBLE,rank_rgt,102,MPI_COMM_WORLD);
    }
    else{
      MPI_Sendrecv(send_buffer_lftu,N_loc,MPI_DOUBLE,rank_lft,102,recv_buffer_rgtu,N_loc,MPI_DOUBLE,rank_rgt,102,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      MPI_Sendrecv(send_buffer_lftv,N_loc,MPI_DOUBLE,rank_lft,102,recv_buffer_rgtv,N_loc,MPI_DOUBLE,rank_rgt,102,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    /*
    ...
    if(rank_row==0){
      ...
      MPI_Recv(recv_buffer_topv,N_loc,MPI_DOUBLE,rank_top,103,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    else if(rank_row==q-1){
      MPI_Send(send_buffer_botu,N_loc,MPI_DOUBLE,rank_bot,103,MPI_COMM_WORLD);
      MPI_Send(send_buffer_botv,N_loc,MPI_DOUBLE,rank_bot,103,MPI_COMM_WORLD);
    }
    else{
      ...
      MPI_Sendrecv(send_buffer_botv,N_loc,MPI_DOUBLE,rank_bot,103,recv_buffer_topv,N_loc,MPI_DOUBLE,rank_top,103,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    ...
    if(rank_row==q-1){
      ...
      MPI_Recv(recv_buffer_botv,N_loc,MPI_DOUBLE,rank_bot,104,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    else if(rank_row==0){
      MPI_Send(send_buffer_topu,N_loc,MPI_DOUBLE,rank_top,104,MPI_COMM_WORLD);
      MPI_Send(send_buffer_topv,N_loc,MPI_DOUBLE,rank_top,104,MPI_COMM_WORLD);
    }
    else{
      ...
      MPI_Sendrecv(send_buffer_topv,N_loc,MPI_DOUBLE,rank_top,104,recv_buffer_botv,N_loc,MPI_DOUBLE,rank_bot,104,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    */
    ...
    */
    ...
    if(rank_col<p-1){ //compute right edge                               
      for(j=1;j<N_loc+1;j++){
        i=N_loc;
...
void write_results(double *u, int N, int N_loc, int nproc, int irank, double h, double L){
  ...
  if(irank==0){
    ...
    //write to file
    FILE *f;
    f = fopen("results_baklery_mpi_3.dat","w"); //open file
    fprintf(f,"x, y, u\n");

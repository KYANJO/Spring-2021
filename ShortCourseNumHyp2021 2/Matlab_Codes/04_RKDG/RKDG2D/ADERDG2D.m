%************************************************************************** 
%% Unstrutured ADER-DG method for 2D hyperbolic conservation laws
...
for n=1:NMAX
    ...
    % ADERDG solver
    uhatav = zeros(nDOF,nVar,nElem);
    dqdtau = zeros(nDOF,nVar,N+1);
    for i=1:nElem
        ...
        iJ = [J(2,2), -J(1,2);
            -J(2,1), J(1,1)];
            iJ = iJ/detJ;  
        fhat = zeros(nDOF,nVar);
        ghat = zeros(nDOF,nVar);
        dqdtau(:,:,i) = uhat(:,:,i);  % zeroth derivative in time
        uhatav(:,:,i) = uhat(:,:,i);    %  the contribution to the average
        % Discrete Cauchy-Kovalevkaya procedure
        coeff = 1;
        for m=0:N-1
            for k=1:nDOF
                [f g] = flux(dqdtau(k,:,m+1));
                fhat(k,:,i) = f';
                ghat(k,:,i) = g';
            end
            % Compute next derivative in time
            dqdtau(:,:,m+1) =  -dt*(ddxi*(iJ(1,1)*fhat(:,:,i) + iJ(1,2)*ghat(:,:,i)) ...
                + ddeta*(iJ(2,1)*fhat(:,:,i) + iJ(2,2)*ghat(:,:,i)));
            % Compute the contribution of the time derivative to the time
            % average
            coeff = coeff/(m+2); % Coefficient for Taylor series
            uhatav(:,:,i) = uhatav(:,:,i) + coeff*dqdtau(:,:,m+1);
        end 
    end
    % ADER-DG
    k1 = LhDG(uhatav);
